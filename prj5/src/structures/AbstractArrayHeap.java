...
/**
 * An {@link AbstractArrayHeap} is an implementation of a heap that represents the tree using an
 * array. By doing this, we can easily maintain the complete tree property required by a Heap.
 *
 * @author jcollard
...
public abstract class AbstractArrayHeap<P, V> {
  ...
  /** The {@link Comparator} which determines priorities in this {@link AbstractArrayHeap}. */
  protected final Comparator<P> comparator;

  /**
   * Creates an {@link AbstractArrayHeap} with the specified {@link Comparator} for determining
   * priority
   *
  ...
  /**
   * Adds the specified {@code value} to this {@link AbstractArrayHeap} with the specified {@code
   * priority}. For convenience, returns the modified {@link AbstractArrayHeap}.
   *
   * @param priority the priority for this entry
   * @param value the value for this entry
   * @return The modified {@link AbstractArrayHeap}
   * @throws NullPointerException if {@code priority} or {@code value} are {@code null}.
   */
  ...
  /**
   * Returns the value that has the highest priority as determined by {@link
   * AbstractArrayHeap#getComparator()}.
   *
   * @return the value that has the highest priority
   * @throws NoSuchElementException if this {@link AbstractArrayHeap} is empty
   */
  public final V peek() {
    if (heap.isEmpty()) {
  ...
  /**
   * Removes the value that has the highest priority as determined by {@link
   * AbstractArrayHeap#getComparator()} from this {@link AbstractArrayHeap}. For convenience, this
   * method returns the removed value.
   *
  ...
  /**
   * Returns {@code true} if this {@link AbstractArrayHeap} has no elements in it and {@code false}
   * otherwise.
   *
   * @return {@code true} if this {@link AbstractArrayHeap} has no elements in it and {@code false}
   *     otherwise.
   */
  ...
  /**
   * Returns an immutable view of the underlying array structure of this {@link AbstractArrayHeap}.
   *
   * @return an immutable view of the underlying array structure of this {@link AbstractArrayHeap}.
   */
  ...
  /**
   * Returns the {@link Comparator} that is used to determine priority in this {@link
   * AbstractArrayHeap}.
   *
   * @return the {@link Comparator} that is used to determine priority in this {@link
   *     AbstractArrayHeap}.
   */
   ...
   *
   * @param ix0 the first index
   * @param ix1 the second index
   * @throws IndexOutOfBoundsException if either index is less than 0 or greater than or equal to
