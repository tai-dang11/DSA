...
/**
 * A {@link PriorityQueue} is a queue data structure that allows elements to be enqueued with a
 * priority such that higher priority elements are dequeued first.
 *
 * @author jcollard
 * @param <P> the priority type of this {@link PriorityQueue}
 * @param <V> the value type of this {@link PriorityQueue}
 */
public interface PriorityQueue<P, V> extends Iterable<Entry<P, V>> {

  /**
   * Enqueues the specified {@code value} into this {@link PriorityQueue} with the specified {@code
   * priority}. This runs in O(log(size)) time. For convenience this method returns the modified
   * {@link PriorityQueue}.
   *
   * @param priority the priority of this enqueue
   * @param value the value being enqueued
   * @return the modified {@link PriorityQueue}.
   * @throws NullPointerException if {@code prioirty} is {@code null} or {@code value} is {@code
   *     null}.
   */
